...
export const WorkspaceCard = memo(({ folder, index, onClick }: WorkspaceCardProps) => {
	...
	return (
		<div
			className="group relative overflow-hidden rounded-2xl border border-void-border-2 bg-gradient-to-br from-void-bg-2/40 to-void-bg-3/40 hover:border-void-accent/40 transition-all duration-300 shadow-sm hover:shadow-xl cursor-pointer active:scale-[0.98] focus:outline-none focus:ring-2 focus:ring-void-accent focus:ring-offset-2"
			onClick={onClick}
		...
		>
			<div className={`absolute inset-0 bg-gradient-to-br ${gradientColor} opacity-0 group-hover:opacity-100 group-focus:opacity-100 transition-opacity duration-500`} />
			<div className="relative p-4 flex items-center gap-4">
				...
				<div className="flex-1 min-w-0">
					<div className="flex items-center gap-2">
						...
						<div className="w-2 h-2 rounded-full bg-emerald-500 shadow-lg shadow-emerald-500/50" aria-hidden="true" />
					</div>
					<span className="text-[10px] text-void-fg-4/60 truncate block mt-0.5 font-mono">{folder.uri.fsPath}</span>
				</div>
