...
export const ActivityItem = memo(({ icon: Icon, title, subtitle, time, status, onClick }: ActivityItemProps) => {
	...
	return (
		<div
			className="group p-4 rounded-xl border border-void-border-2 bg-void-bg-2 hover:bg-void-bg-3 hover:border-void-border-1 transition-all cursor-pointer focus:outline-none focus:ring-2 focus:ring-void-accent focus:ring-offset-2"
			onClick={onClick}
		...
		>
			<div className="flex items-start gap-4">
				<div className="flex-shrink-0 w-10 h-10 rounded-xl bg-void-bg-3 border border-void-border-2 flex items-center justify-center group-hover:border-void-accent/40 group-focus:border-void-accent/40 transition-all">
				...
				<div className="flex-1 min-w-0">
					...
					<p className="text-[10px] text-void-fg-4 mt-1 truncate font-mono">{subtitle}</p>
				</div>
				<div className="flex flex-col items-end gap-2 flex-shrink-0">
					<span className="text-[10px] font-bold text-void-fg-4 uppercase tracking-tighter">{time}</span>
